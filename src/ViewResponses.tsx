        ...
        {error ? (
          ...
          <div className="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-200 flex flex-col">
            ...
            <div className="flex gap-2 border-b border-gray-200 pb-2 pt-2 px-4 font-mono text-sm overflow-x-auto shrink-0 w-full" data-lenis-prevent="true">
              ...
              >
                New ({responses.filter(r => (r.status || 'applied') === 'applied').length})
              </button>
              <button
              ...
              >
                Approved ({responses.filter(r => ['approved', 'participated', 'missed'].includes(r.status)).length})
              </button>
              <button
              ...
              >
                Finalists ({responses.filter(r => r.status === 'finalized').length})
              </button>
              <button
                onClick={() => setAppStatusFilter('deleted')}
                className={`px-4 py-2 rounded-t transition-colors whitespace-nowrap ${appStatusFilter === 'deleted' ? 'bg-[#f9fafb] border border-b-0 border-gray-200 font-bold text-purple-700' : 'text-gray-500 hover:text-black'}`}
              >
                Trash Bin ({responses.filter(r => r.status === 'deleted').length})
              </button>
            </div>
                  ...
                  <table className="w-full text-left border-collapse text-sm">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {responses.filter(r => {
                        ...
                        if (!name || name.trim() === '') {
                          const firstKey = response.formData ? Object.keys(response.formData)[0] : null;
                          if (firstKey && response.formData[firstKey]) {
                            const val = response.formData[firstKey];
                            name = Array.isArray(val) ? val.join(', ') : String(val);
                          } else {
                            name = 'Unknown';
                          }
                        }
                                ...
                                <button
                                  onClick={async () => {
                                    if (window.confirm('Are you sure you want to move this response to the Trash Bin?')) {
                                      try {
                                        await updateDoc(doc(db, 'volunteerApplications', response.id), { status: 'deleted' });
                                        setResponses(apps => apps.map(a => a.id === response.id ? { ...a, status: 'deleted' } : a));
                                      } catch (err) {
                                        console.error('Error moving response to trash:', err);
                                  ...
                                  <button
                                    onClick={async () => {
                                      if (window.confirm('Restore this response to New (Pending)?')) {
                                        try {
                                          await updateDoc(doc(db, 'volunteerApplications', response.id), { status: 'applied' });
                                          setResponses(apps => apps.map(a => a.id === response.id ? { ...a, status: 'applied' } : a));
                                        } catch (err) {
                                          console.error('Error recovering response:', err);
                                          alert('Failed to recover response. You may not have permission.');
                                        }
                                      }
                                    }}
                                    className="text-xs bg-blue-100 text-blue-700 px-3 py-1.5 rounded hover:bg-blue-200 transition-colors mr-2 uppercase tracking-wider"
                                  >
                                    🔄 Recover
                                  </button>
                                  <button
